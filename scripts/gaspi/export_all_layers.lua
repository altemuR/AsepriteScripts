...
dlg:entry{
    ...
    text = "{layergroups}{layername}_{datetime}"
}
dlg:combobox{
    id = 'format',
    label = 'Export Format:',
...
filename = filename:gsub("{date}", os.date("%Y-%m-%d"))
filename = filename:gsub("{time}", os.date("%H-%M-%S"))
filename = filename:gsub("{datetime}", os.date("%Y-%m-%d_%H-%M-%S"))
